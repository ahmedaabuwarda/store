...
<tbody>
  ...
  <tr>
    ...
    <td class="display-3 text-center">{{ $sanadat_sarf->balance }}<i class="fas fa-shekel-sign ml-1"></i></td>
    <td class="display-3 text-center">{{ $sanadat_sarf->byan }}</td>
    <td class="display-3 text-center">
      <button type="submit" class="btn btn-sm text-dark delete_sanadat_sarf_button" data-toggle="tooltip" data-placement="top" title="حذف" data-dataid="{{ $sanadat_sarf->id }}" style="background-color: #FFB740;"><i class="fa fa-trash"></i></button>
    </td>
  </tr>
  @php $i++; @endphp
  @endforeach
