...
<div class="container-fluid mt--8">
      ...
      <div class="card">
        <div class="card-header border-0">
          <div class="row align-items-center">
            <div class="col">
              <h3 class="mb-0">الايتام</h3>
            </div>
            <div class="col-xl-8 col-md-12 text-center">
              <input type="text" name="search_input" id="search_input" class="form-control" placeholder="...ابحث عن يتيم">
            </div>
            <div class="col-xl-3 col-md-12 text-right">
              ...
              <a class="btn text-white btn-dark" data-toggle="modal" data-target="#create_orphan_modal"><i class="fa fa-plus"></i> اضافة يتيم</a>
            </div>
          </div>
        </div>
        <div class="table-responsive">
          <!-- orphan table -->
          <table class="table tablee align-items-center table-flush table-hover" id="orphan_table">
            @include('admin.orphan.table')
          </table>
        </div>
      </div>
    </div>
  ...
  <nav aria-label="..." class="justify-content-center">
    <ul class="pagination justify-content-center">
      ...
      @for($p = 1; $p <= $pages; $p++)
        <li class="page-item @if(Request::fullUrl() == URL('/orphan?page=' . $p)) active @endif"><a class="page-link" href="{{ URL('/orphan?page=' . $p) }}">{{ $p }}</a></li>
        @endfor
        <li class="page-item">
...
<div class="modal fade" id="create_orphan_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="create_orphan_form">
        <div class="modal-body">
          @csrf
          <ul>
            <li>ملف الاكسل: الرقم | الكفيل | اليتيم | الوصي | رقم هاتف اليتيم | الشهر | الدفعة | ملاحظات</li>
          </ul>
          <div class="row">
            <div class="col-12">
              ...
              <div class="form-group">
                <label class="form-control-label">استيراد ايتام</label>
...
<script>
  ...
  $('#create_orphan_form').submit(function(e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status == "success") {
          ...
          // refresh the table
          get_orphans();
          $('#create_orphan_form')[0].reset();
          $('#create_orphan_modal').modal('hide');
        } else {
          Swal.fire(
            'عفواً',
  ...
  $('#orphan_table').on('click', '.table_from_to_pdf_button', function(e) {
    let from_to = $(this).data('fromto');
    $('#from_to_pdf_modal').modal('show');
    $('#from_to').val(from_to);
  });
  $('#orphan_table').on('click', '.edit_button', function(e) {
    e.preventDefault();
    let id = $(this).data('id');
    $.ajax({
      url: "/orphan/edit",
      ...
      success: function(response) {
        if (response.status == "success") {
          $('#create_orphan_modal').modal('show');
          $('#exampleModalLabel').text('تعديل عملة');
          $('#create_orphan_modal .row').html(response.modal);
        } else {
          Swal.fire(
            'عفواً',
            'حدث خطأ ما',
  ...
  function get_orphans() {
    $.ajax({
      ...
      success: function(response) {
        $('#orphan_table').html('');
        $('#orphan_table').append(response.table);
      },
      error: function(response) {
        Swal.fire(
          'خطأ',
