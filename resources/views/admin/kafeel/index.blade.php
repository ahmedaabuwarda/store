...
<div class="container-fluid mt--8">
      ...
      <div class="card">
        <div class="card-header border-0">
          <div class="row align-items-center">
            <div class="col">
              <h3 class="mb-0">الكفلاء</h3>
            </div>
            <div class="col-xl-8 col-md-12 text-center">
              <input type="text" name="search_input" id="search_input" class="form-control" placeholder="...ابحث عن كفيل">
            </div>
            <div class="col-xl-3 col-md-12 text-right">
              ...
              <a class="btn text-white btn-dark" data-toggle="modal" data-target="#create_kafeel_modal"><i class="fa fa-plus"></i> اضافة كفيل</a>
            </div>
          </div>
        </div>
        <div class="table-responsive">
          <!-- kafeel table -->
          <table class="table tablee align-items-center table-flush table-hover" id="kafeel_table">
            @include('admin.kafeel.table')
          </table>
        </div>
      </div>
    </div>
  ...
  <nav aria-label="..." class="justify-content-center">
    <ul class="pagination justify-content-center">
      ...
      @for($p = 1; $p <= $pages; $p++)
        <li class="page-item @if(Request::fullUrl() == URL('/kafeel?page=' . $p)) active @endif"><a class="page-link" href="{{ URL('/kafeel?page=' . $p) }}">{{ $p }}</a></li>
        @endfor
        <li class="page-item">
...
<div class="modal fade" id="create_kafeel_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">اضافة كفيل جديد</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="create_kafeel_form">
            ...
            <div class="col-12">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="اسم الكفيل" value="{{ old('name') }}" autocomplete="name" autofocus>
                </div>
                @error('name')
              ...
              <div class="form-group">
                <label class="form-control-label">استيراد كفلاء</label>
...
<script>
  ...
  $('#create_kafeel_form').submit(function(e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status == "success") {
          ...
          // refresh the table
          get_kafeels();
          $('#create_kafeel_form')[0].reset();
          $('#create_kafeel_modal').modal('hide');
        } else {
          Swal.fire(
            'عفواً',
            response.message,
  ...
  $('#kafeel_table').on('click', '.edit_button', function(e) {
    e.preventDefault();
    let id = $(this).data('id');
    $.ajax({
      url: "/kafeel/edit",
      ...
      success: function(response) {
        if (response.status == "success") {
          $('#create_kafeel_modal').modal('show');
          $('#exampleModalLabel').text('تعديل عملة');
          $('#create_kafeel_modal .row').html(response.modal);
        } else {
          Swal.fire(
            'عفواً',
            'حدث خطأ ما',
  ...
  // get all customers
  function get_kafeels() {
    $.ajax({
      url: "/kafeels",
      type: "GET",
      success: function(response) {
        $('#kafeel_table').html('');
        $('#kafeel_table').append(response.table);
      },
      error: function(response) {
        Swal.fire(
          'خطأ',
