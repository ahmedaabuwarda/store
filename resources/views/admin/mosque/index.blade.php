...
<div class="container-fluid mt--8">
      ...
      <div class="card">
        <div class="card-header border-0">
          <div class="row align-items-center">
            <div class="col">
              <h3 class="mb-0">المساجد</h3>
            </div>
            <div class="col-xl-8 col-md-12 text-center">
              <input type="text" name="search_input" id="search_input" class="form-control" placeholder="...ابحث عن مسجد">
            </div>
            <div class="col-xl-3 col-md-12 text-right">
              ...
              <a class="btn text-white btn-dark" data-toggle="modal" data-target="#create_mosque_modal"><i class="fa fa-plus"></i> اضافة مسجد</a>
            </div>
          </div>
        </div>
        <div class="table-responsive">
          <!-- mosque table -->
          <table class="table tablee align-items-center table-flush table-hover" id="mosque_table">
            @include('admin.mosque.table')
          </table>
        </div>
      </div>
    </div>
  ...
  <nav aria-label="..." class="justify-content-center">
    <ul class="pagination justify-content-center">
      ...
      @for($p = 1; $p <= $pages; $p++)
        <li class="page-item @if(Request::fullUrl() == URL('/mosque?page=' . $p)) active @endif"><a class="page-link" href="{{ URL('/mosque?page=' . $p) }}">{{ $p }}</a></li>
        @endfor
        <li class="page-item">
...
<div class="modal fade" id="create_mosque_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="col-12">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="اسم المسجد" value="{{ old('name') }}" autocomplete="name">
                </div>
                @error('name')
              ...
              <div class="form-group">
                <label class="form-control-label">استيراد مساجد</label>
...
<script>
  ...
  $('#create_mosque_form').submit(function(e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status == "success") {
          ...
          // refresh the table
          get_curencies();
          $('#create_mosque_form')[0].reset();
          $('#create_mosque_modal').modal('hide');
        } else {
          Swal.fire(
            'عفواً',
            response.message,
  ...
  $('#mosque_table').on('click', '.edit_button', function(e) {
    e.preventDefault();
    let id = $(this).data('id');
    $.ajax({
      url: "/mosque/edit",
      ...
      success: function(response) {
        if (response.status == "success") {
          $('#create_mosque_modal').modal('show');
          $('#exampleModalLabel').text('تعديل عملة');
          $('#create_mosque_modal .row').html(response.modal);
        } else {
          Swal.fire(
            'عفواً',
            response.message,
  ...
  function get_curencies() {
    $.ajax({
      ...
      success: function(response) {
        $('#mosque_table').html('');
        $('#mosque_table').append(response.table);
      },
      error: function(response) {
        Swal.fire(
          'خطأ',
