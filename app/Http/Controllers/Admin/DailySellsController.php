...
class DailySellsController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $export_ainiats = ExportAiniat::select('id', 'number', 'date_created', 'byan', 'provider_id', 'customer_id', 'worker_id', 'remaining_balance', 'paid_balance', 'total_profit')->where('customer_id', 1)->with('worker:id,name')->with('customer:id,name')->with('provider:id,name')->orderBy('id', 'DESC')->paginate($page);

    $pages = ceil(ExportAiniat::count() / $page);
    $box = DB::select('SELECT remaining from box where id IN (3,7);');
    if ($request->ajax()) {
      $table = view('admin.daily_sells.table', compact('export_ainiats'))->render();
      return response()->json(['status' => 'success', 'table' => $table]);
    } else {
      return view('admin.daily_sells.index', compact('export_ainiats', 'pages', 'box'));
    }
  }
  ...
  public function store(Request $request)
  {
    ...
    try {
      ...
      $export_ainiat->remaining_balance = 0;
      $export_ainiat->discount = 0;
      $export_ainiat->total_balance = abs($remaining_balance) + $paid_balance;
      if ($request['byan'] == null) {
        $export_ainiat->byan = 'لا يوجد';
      } else {
        $export_ainiat->byan = $request['byan'];
      }
      ...
      DB::commit();
      return redirect('/daily_sell/edit/' . $export_ainiat->id);
    } catch (Exception $e) {
      DB::rollBack();
      return redirect('/daily_sell/edit/' . $export_ainiat->id);
    }
  ...
  {
    $export_ainiat = ExportAiniat::select('id', 'number', 'date_created', 'provider_id', 'customer_id', 'worker_id', 'total_balance', 'total_profit', 'paid_balance', 'remaining_balance', 'discount', 'byan')->with('sold_product:id,product_id,quantity,sell_price,total_price,profit,export_ainiat_id')->where('id', $id)->first();
    $products = DB::select('SELECT id, name, original_price, quantity FROM products ORDER BY id DESC');
    $error = 1;
    return view('admin.daily_sells.edit', compact('export_ainiat', 'products', 'error'));
  }
  ...
  public function update(Request $request, $id)
  {
    ...
    DB::beginTransaction();
    try {
      if ($request['tbl'] == null && $export_ainiat->paid_balance != $paid_balance) {
        return redirect('/daily_sells');
      } else {
  ...
  public function delete_product($id)
  {
    ...
    try {
      ...
      DB::commit();
      return redirect('/daily_sell/edit/' . $sold_product->export_ainiat_id);
    } catch (\Exception $e) {
      DB::rollBack();
      return redirect('/daily_sells')->with('error', 'Error: ' . $e->getMessage());
    }
  ...
  public function to_pdf(Request $request)
  {
    $from = $request['from'];
    $to = $request['to'];
    $export_ainiats = ExportAiniat::select('id', 'number', 'date_created', 'byan', 'provider_id', 'customer_id', 'worker_id', 'remaining_balance', 'paid_balance', 'total_profit')->with('worker:id,name')->with('customer:id,name')->with('provider:id,name')->whereRaw('date_created >= ? AND date_created <= ? AND customer_id = 1', [$from, $to])->orderBy('id', 'DESC')->get();
